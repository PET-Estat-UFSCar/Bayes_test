...
def validate_param(value, min_val=None, max_val=None, min_exclusive=False, max_exclusive=False):
    ...
    if min_val is not None:
        if min_exclusive and value <= min_val: return False, error_message
        if not min_exclusive and value < min_val: return False, error_message
    if max_val is not None:
        if max_exclusive and value >= max_val: return False, error_message
        if not max_exclusive and value > max_val: return False, error_message
            
    return True, ""

@app.callback(
    [Output('input-a', 'className'), Output('error-msg-a', 'children')],
...
def validate_input_a(value, priori):
    ...
    if priori in ["Beta", "Gama"]:
        is_valid, message = validate_param(value, min_val=0, min_exclusive=True)
        if not is_valid:
            return f"{base_class} is-invalid", message
    return base_class, ""

@app.callback(
    [Output('input-b', 'className'), Output('error-msg-b', 'children')],
...
@app.callback(
    [Output('input-c', 'className'), Output('error-msg-c', 'children')],
    Input('input-c', 'value'), State('prioris', 'value'), prevent_initial_call=True)
def validate_input_c(value, priori):
    base_class = "styled-input"
    if priori == "Normal-Gama":
        is_valid, message = validate_param(value, min_val=1, min_exclusive=True)
        if not is_valid:
            return f"{base_class} is-invalid", message
    return base_class, ""

@app.callback(
    [Output('input-d', 'className'), Output('error-msg-d', 'children')],
...
@app.callback(
    [Output('input-tamanho', 'className'), Output('error-msg-tamanho', 'children')],
...
def validate_input_tamanho(value, verossimilhanca):
    ...
    is_valid, message = validate_param(value, min_val=min_val)
    if not is_valid:
        return f"{base_class} is-invalid", message
    return base_class, ""

@app.callback(
    [Output('input-x-bernoulli', 'className'), Output('error-msg-x-bernoulli', 'children')],
...
def validate_input_x_bernoulli(value, verossimilhanca):
    base_class = "styled-input"
    if verossimilhanca == "Bernoulli":
        is_valid, message = validate_param(value, min_val=0, max_val=1)
        if not is_valid:
            return f"{base_class} is-invalid", message
    return base_class, ""

@app.callback(
    [Output('input-conhecido', 'className'), Output('error-msg-conhecido', 'children')],
...
@app.callback(
    [Output('input-m', 'className'), Output('error-msg-m', 'children')],
    Input('input-m', 'value'), State('verossimilhancas', 'value'), prevent_initial_call=True)
def validate_input_m(value, verossimilhanca):
    base_class = "styled-input"
    if verossimilhanca == "Binomial negativa":
        is_valid, message = validate_param(value, min_val=1)
        if not is_valid:
            return f"{base_class} is-invalid", message
    return base_class, ""

@app.callback(
    [Output('input-x', 'className'), Output('error-msg-x', 'children')],
...
def validate_input_x_and_m(x, m, verossimilhanca):
    ...
    if verossimilhanca == "Binomial negativa":
        ...
        is_valid, message = validate_param(x, min_val=0)
        if not is_valid: return f"{base_class} is-invalid", message
