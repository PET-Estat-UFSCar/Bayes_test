...
# ... (Todo o seu código da Seção 3 permanece aqui, inalterado) ...
...
def update_verossimilhanca_dropdown(priori):
    ...
    if priori == "Beta":
        options = ["Bernoulli", "Geométrica", "Binomial negativa", "Binomial"]
    elif priori == "Gama":
...
def render_priori_params(priori):
    ...
    if priori == "Beta":
        ...
        return html.Div([
            ...
            html.Div(id='error-c', className='error-message'),
            html.Label("Parâmetro de taxa β (> 0):"),
            dcc.Input(id='input-d', type='number', value=2, className="styled-input"),
            html.Div(id='error-d', className='error-message'),
        ])
    return []
...
def render_verossimilhanca_params(verossimilhanca):
    """CORRIGIDO: Renderiza os inputs da verossimilhança, garantindo que todos os
    elementos (visíveis ou ocultos) existam no layout para evitar erros de callback."""
    ...
    # Define todos os grupos de input possíveis com suas divs de erro correspondentes
    input_x_group = html.Div([
        dcc.Input(id='input-x', type='number', value=1, className="styled-input"),
        html.Div(id='error-x', className='error-message')
    ])
    input_x_bernoulli_group = html.Div([
        dcc.Input(id='input-x-bernoulli', type='number', step=0.01, min=0, max=1, value=0.5, className="styled-input"),
        html.Div(id='error-x-bernoulli', className='error-message')
    ])
    input_m_group = html.Div([
        dcc.Input(id='input-m', type='number', value=10, className="styled-input"),
        html.Div(id='error-m', className='error-message')
    ])
    input_conhecido_group = html.Div([
        dcc.Input(id='input-conhecido', type='number', value=1, className="styled-input"),
        html.Div(id='error-conhecido', className='error-message')
    ])

    # Caso especial para Bernoulli, que usa um input diferente
    if verossimilhanca == "Bernoulli":
        return html.Div([
            html.Label("Média amostral (0 ≤ x̄ ≤ 1):"),
            input_x_bernoulli_group,
            *base_inputs,
            # Oculta os outros inputs para evitar conflitos
            html.Div(input_x_group, style={'display': 'none'}),
            html.Div(input_m_group, style={'display': 'none'}),
            html.Div(input_conhecido_group, style={'display': 'none'}),
        ])

    # Layout padrão para as outras distribuições
    specific_section = []
    hidden_inputs = []
    media_label = "Média amostral (x̄):"
    
    # Oculta o input de Bernoulli por padrão nos outros casos
    hidden_inputs.append(html.Div(input_x_bernoulli_group, style={'display': 'none'}))

    if verossimilhanca == "Binomial":
        media_label = "Média amostral (0 ≤ x̄ ≤ m):"
        specific_section = [html.Label("Número de ensaios (m ≥ 1):"), input_m_group]
        hidden_inputs.append(html.Div(input_conhecido_group, style={'display': 'none'}))
    elif verossimilhanca == "Binomial negativa":
        media_label = "Média amostral (x̄ > r):"
        specific_section = [html.Label("Número de sucessos (r ≥ 1):"), input_m_group]
        hidden_inputs.append(html.Div(input_conhecido_group, style={'display': 'none'}))
    elif verossimilhanca == "Gama (b desconhecido)":
        media_label = "Média amostral (x̄ > 0):"
        specific_section = [html.Label("Parâmetro 'a' conhecido (>0):"), input_conhecido_group]
        hidden_inputs.append(html.Div(input_m_group, style={'display': 'none'}))
    elif verossimilhanca == "Normal (média desconhecida)":
        media_label = "Média amostral (x̄ ∈ ℝ):"
        specific_section = [html.Label("Variância populacional (σ²) conhecida (>0):"), input_conhecido_group]
        hidden_inputs.append(html.Div(input_m_group, style={'display': 'none'}))
    elif verossimilhanca == "Normal (média e precisão desconhecidas)":
        media_label = "Média amostral (x̄ ∈ ℝ):"
        specific_section = [html.Label("Variância amostral (s² > 0):"), input_conhecido_group]
        hidden_inputs.append(html.Div(input_m_group, style={'display': 'none'}))
    else:  # Geométrica, Poisson, Exponencial
    ...
    final_layout = [html.Label(media_label), input_x_group, *specific_section, *base_inputs, *hidden_inputs]
...
def get_validation_states(a, b, c, d, x, m, n, conhecido, x_bernoulli, priori, verossimilhanca):
    """CORRIGIDO: Função central que verifica a validade de todos os parâmetros
    e retorna um dicionário de status detalhado."""
    is_valid = {
        'a': True, 'b': True, 'c': True, 'd': True,
        'x': True, 'x_bernoulli': True, 'm': True, 'n': True, 'conhecido': True,
        'geral': True
    }
    
    if priori is None or verossimilhanca is None:
        is_valid['geral'] = False
        return is_valid

    # Valida parâmetros da Priori
    if a is None or b is None: is_valid['geral'] = False
    if priori == "Beta" or priori == "Gama":
        if a is not None and a <= 0: is_valid['a'] = False
        if b is not None and b <= 0: is_valid['b'] = False
    elif priori == "Normal":
        if b is not None and b <= 0: is_valid['b'] = False
    elif priori == "Normal-Gama":
        if c is None or d is None: is_valid['geral'] = False
        if b is not None and b <= 0: is_valid['b'] = False
        if c is not None and c <= 1: is_valid['c'] = False
        if d is not None and d <= 0: is_valid['d'] = False

    # Valida parâmetros da Verossimilhança
    if n is None: 
        is_valid['geral'] = False
    else:
        tamanho_min = 2 if verossimilhanca == "Normal (média e precisão desconhecidas)" else 1
        if n < tamanho_min: is_valid['n'] = False

    if verossimilhanca == 'Bernoulli':
        if x_bernoulli is None: 
            is_valid['geral'] = False
        elif not (0 <= x_bernoulli <= 1):
             is_valid['x_bernoulli'] = False
    else:
        if x is None: is_valid['geral'] = False
        
        if verossimilhanca == "Binomial":
            if m is None: is_valid['geral'] = False
            if m is not None and m < 1: is_valid['m'] = False
            if x is not None and m is not None and not (0 <= x <= m): is_valid['x'] = False
        elif verossimilhanca == "Binomial negativa":
            if m is None: is_valid['geral'] = False
            if m is not None and m < 1: is_valid['m'] = False
            if x is not None and m is not None and not (x > m): is_valid['x'] = False
        elif verossimilhanca == "Geométrica":
            if x is not None and x < 1: is_valid['x'] = False
        elif verossimilhanca in ["Poisson", "Exponencial"]:
            if x is not None and x <= 0: is_valid['x'] = False
        elif verossimilhanca == "Gama (b desconhecido)":
            if conhecido is None: is_valid['geral'] = False
            if x is not None and x <= 0: is_valid['x'] = False
            if conhecido is not None and conhecido <= 0: is_valid['conhecido'] = False
        elif verossimilhanca in ["Normal (média desconhecida)", "Normal (média e precisão desconhecidas)"]:
            if conhecido is None: is_valid['geral'] = False
            if conhecido is not None and conhecido <= 0: is_valid['conhecido'] = False

    # Checagem final da validade geral
...
@app.callback(
    [Output('input-a', 'className'), Output('error-a', 'children'),
     ...
     Output('input-x-bernoulli', 'className'), Output('error-x-bernoulli', 'children'),
    ...
    [Input('input-a', 'value'), Input('input-b', 'value'),
     ...
     Input('input-x', 'value'), Input('input-x-bernoulli', 'value'),
     Input('input-m', 'value'), Input('input-tamanho', 'value'),
     Input('input-conhecido', 'value')],
    [State('prioris', 'value'), State('verossimilhancas', 'value')]
)
def validate_inputs_and_set_styles(a, b, c, d, x, x_bernoulli, m, n, conhecido, priori, verossimilhanca):
    """CORRIGIDO: Valida todos os inputs e retorna as classes CSS e mensagens de erro."""
    
    # Previne a atualização no carregamento inicial antes de qualquer interação
    ...
    error_messages = {
        ...
        'b_normal': "A variância deve ser > 0.",
        'b_normal_gama': "λ deve ser > 0.",
        'c': "α deve ser > 1.",
        'd': "β deve ser > 0.",
        'x_binomial': "A média deve estar no intervalo [0, m].",
        'x_bn': "A média deve ser > r.",
        'x_geo': "A média deve ser ≥ 1.",
        'x_pos_exp_gama': "A média deve ser > 0.",
        'x_bernoulli': "A média deve estar no intervalo [0, 1].",
        'm': "O valor deve ser ≥ 1.",
        'n': f"O tamanho da amostra deve ser ≥ {2 if verossimilhanca == 'Normal (média e precisão desconhecidas)' else 1}.",
        'conhecido_gama': "O parâmetro 'a' deve ser > 0.",
        'conhecido_normal': "A variância deve ser > 0."
    }
    
    # Função auxiliar para gerar o par (classe, erro)
    def get_style_and_error(key, msg):
        return (invalid_class, html.P(msg)) if not validation.get(key, True) else (valid_class, None)

    # Determina a mensagem de erro correta para cada campo dinâmico
    b_msg = error_messages['b_beta_gama']
    ...
    # Gera todos os outputs na ordem correta
    out_a = get_style_and_error('a', error_messages['a'])
    out_b = get_style_and_error('b', b_msg)
    out_c = get_style_and_error('c', error_messages['c'])
    out_d = get_style_and_error('d', error_messages['d'])
    out_x = get_style_and_error('x', x_msg)
    out_x_bernoulli = get_style_and_error('x_bernoulli', error_messages['x_bernoulli'])
    out_m = get_style_and_error('m', error_messages['m'])
    out_n = get_style_and_error('n', error_messages['n'])
    out_conhecido = get_style_and_error('conhecido', conhecido_msg)

    return [
        *out_a, *out_b, *out_c, *out_d, *out_x, *out_x_bernoulli,
        *out_m, *out_n, *out_conhecido
    ]
...
def update_all_graphs(a, b, c, d, m, x, x_bernoulli, n, conhecido, prioris, verossimilhancas):
    ...
    elif verossimilhancas == "Geométrica": fig_vero = verossimilhanca_geometrica_aproximada(x, n)
    elif verossimilhancas == "Binomial negativa": fig_vero = verossimilhanca_binomial_negativa_aproximada(x, m, n)
    elif verossimilhancas == "Exponencial": fig_vero = verossimilhanca_exponencial_aproximada(x, n)
    elif verossimilhancas == "Poisson": fig_vero = verossimilhanca_poisson_aproximada(x, n)
    elif verossimilhancas == "Gama (b desconhecido)": fig_vero = verossimilhanca_gama_aproximada(x, conhecido, n)
    ...
    elif verossimilhancas == "Geométrica": fig_post = posteriori_beta(a + n, round(b + n * (x - 1), 3))
    elif verossimilhancas == "Binomial negativa": fig_post = posteriori_beta(round(a + n * m, 3), round(b + n * (x - m), 3))
    elif verossimilhancas == "Exponencial": fig_post = posteriori_gama(a + n, round(b + n * x, 3))
    elif verossimilhancas == "Poisson": fig_post = posteriori_gama(round(n * x + a, 3), b + n)
    elif verossimilhancas == "Gama (b desconhecido)": fig_post = posteriori_gama(round(a + n * conhecido, 3), round(b + n * x, 3))
    ...
    elif verossimilhancas == "Geométrica": fig_conjunto = beta_geometrica(a, b, x, n)
    elif verossimilhancas == "Binomial negativa": fig_conjunto = beta_binomial_negativa(a, b, x, m, n)
    elif verossimilhancas == "Exponencial": fig_conjunto = gama_exponencial(a, b, x, n)
    elif verossimilhancas == "Poisson": fig_conjunto = gama_poisson(a, b, x, n)
    elif verossimilhancas == "Gama (b desconhecido)": fig_conjunto = gama_gama(a, b, x, conhecido, n)
    elif verossimilhancas == "Normal (média desconhecida)": fig_conjunto = normal_normal(a, b, x, conhecido, n)
    elif verossimilhancas == "Normal (média e precisão desconhecidas)": fig_conjunto = Normal_Gama_final(a, b, c, d, x, conhecido, n)

    return fig_priori, fig_vero, fig_post, fig_conjunto, style

# ... (O restante do seu código, incluindo o callback das fórmulas e o __main__, permanece inalterado) ...
@app.callback(
