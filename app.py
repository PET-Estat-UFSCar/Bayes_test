...
def render_verossimilhanca_params(verossimilhanca):
    ...
    if verossimilhanca == "Binomial":
        ...
        if verossimilhanca == "Geométrica":
            ...
            error_div = html.Div(id='error-message-div')
        elif verossimilhanca == "Poisson" or verossimilhanca == "Exponencial":
            media_label = "Média amostral (x̄ > 0):"
            error_div = html.Div(id='error-message-div')
    ...
    final_layout = [
        ...
        input_x,
        *specific_section,
        error_div, # Adiciona o container de erro
        *base_inputs,
...
def is_invalid(verossimilhanca, x, m, n, conhecido):
    """Verifica se os parâmetros da verossimilhança são inválidos."""
    if x is None or n is None: return True
    if verossimilhanca == "Binomial":
        return m is None or not (0 <= x <= m)
    if verossimilhanca == "Binomial negativa":
        return m is None or not (x > m)
    if verossimilhanca == "Geométrica":
        return x < 1
    if verossimilhanca in ["Poisson", "Exponencial", "Gama (b desconhecido)"]:
        if verossimilhanca == "Gama (b desconhecido)" and conhecido is None:
             return True
        return x <= 0
    if verossimilhanca == "Normal (média desconhecida)":
        return conhecido is None or conhecido <= 0
    if verossimilhanca == "Normal (média e precisão desconhecidas)":
        return conhecido is None or conhecido <= 0 or n < 2
    return False
...
def display_error_message(x, m, verossimilhanca):
    """Exibe mensagens de erro dinâmicas para os parâmetros da verossimilhança."""
    if x is None:
        return None # Não mostra erro se o campo estiver vazio

    msg_style = {'color': 'red', 'fontSize': 'small', 'marginTop': '5px'}
    if verossimilhanca == "Binomial" and m is not None and not (0 <= x <= m):
        return html.P("A média amostral (x̄) deve estar no intervalo [0, m].", style=msg_style)
    if verossimilhanca == "Binomial negativa" and m is not None and not (x > m):
        return html.P("A média amostral (x̄) deve ser maior que o número de sucessos (r).", style=msg_style)
    if verossimilhanca == "Geométrica" and x < 1:
        return html.P("A média amostral (x̄) deve ser maior ou igual a 1.", style=msg_style)
    if verossimilhanca in ["Poisson", "Exponencial", "Gama (b desconhecido)"] and x <= 0:
        return html.P("A média amostral (x̄) deve ser maior que 0.", style=msg_style)

    return None # Sem erro, não retorna nada
...
def update_likelihood_graph(m, x, x_bernoulli, n, conhecido, verossimilhancas):
    ...
    if any(p is None for p in [x, x_bernoulli, n, conhecido, verossimilhancas]):
        raise exceptions.PreventUpdate

    style = {**CARD_STYLE}
    if is_invalid(verossimilhancas, x, m, n, conhecido):
        return go.Figure(layout={"template": "plotly_white"}), style # Retorna gráfico vazio se inválido
...
def update_posterior_graph(a, b, c, d, m, x, x_bernoulli, n, conhecido, prioris, verossimilhancas):
    ...
    if any(p is None for p in [a, b, c, d, x, x_bernoulli, n, conhecido, prioris, verossimilhancas]):
        raise exceptions.PreventUpdate
...
def update_final_graph(a, b, c, d, m, x, x_bernoulli, n, conhecido, prioris, verossimilhancas):
    ...
    if is_invalid(verossimilhancas, x, m, n, conhecido):
        return go.Figure(layout={"title": "Ajuste os parâmetros para gerar o gráfico", "template": "plotly_white"})
